...
class LsApp
  ...
  def display_long
    @files.each do |file|
      puts "#{file.type}#{file.permission} #{file.nlink.to_s.rjust(max_nlink_count, ' ')} #{file.owner} #{file.group} #{file.size.to_s.rjust(max_file_size_count, ' ')} #{file.modified_time} #{file.name}"
    end
  end
